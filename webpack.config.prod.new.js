...
class ReplacePathsPlugin {
  ...
  containsBasePath(filePath) {
    ...
    const normalizePath = filePath.startsWith('/') ? filePath : '/' + filePath;
    return normalizePath.startsWith(this.basePath) || normalizePath.includes(this.basePath);
  }
  ...
  replacePathsInFile(filePath) {
    ...
    if (isCss) {
      ...
      content = content.replace(/(href|src|content|poster)=["'](\/(?!\/))([^"']+)["']/g, (match, attr, slash, filePath) => {
        ...
        if (filePath.startsWith('ru-UA/') || filePath.startsWith('uk-UA/')) {
          return match;
        }
  ...
  apply(compiler) {
    compiler.hooks.afterEmit.tap('ReplacePathsPlugin', (compilation) => {
      ...
      const htmlUkFile = path.join(outputPath, 'uk-UA', 'index.html');
      this.replacePathsInFile(htmlUkFile);

      const htmlRuFile = path.join(outputPath, 'ru-UA', 'index.html');
...
module.exports = (env = {}) => {
  ...
  return merge(common, {
    ...
    plugins: [
      ...
      // index.html -> /uk-UA/index.html
      new HtmlWebpackPlugin({
        template: './index.html',
        filename: 'uk-UA/index.html',
        publicPath: basePath ? `${basePath}/` : '/',
        inject: false, // Вимкнути автоматичну інжекцію скриптів (скрипт вже є в HTML)
        templateParameters: {
          GTM_ID: gtmId || false,
      ...
      // index.ru.html -> /ru-UA/index.html
      new HtmlWebpackPlugin({
        template: './index.ru.html',
        filename: 'ru-UA/index.html',
        publicPath: basePath ? `${basePath}/` : '/',
        inject: false, // Вимкнути автоматичну інжекцію скриптів (скрипт вже є в HTML)
        templateParameters: {
          GTM_ID: gtmId || false,
